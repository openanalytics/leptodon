use leptos::{
    either::Either,
    prelude::{ClassAttribute, Effect, ElementChild, For, Get, IntoAny, Set},
};
use std::sync::Arc;
...
pub fn GroupedTable() -> impl IntoView {
    ...
    Effect::new(move || {
        pagination_controller.current_page.set(visible_current_page.get() - 1);
    });
    view! {
        <div> hello </div>
        <table>
            <TableContent rows display_strategy=strat scroll_container="html"></TableContent>
        </table>
        { move || {
