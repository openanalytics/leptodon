...
use leptos::prelude::{AriaAttributes, CustomAttribute, GlobalAttributes};
use leptos::{IntoView, component, view};

const BADGE_BASE_CLASSES: &str = "flex font-medium rounded-lg";
...
pub fn Badge(
    ...
    #[prop(optional, into)] prefix: MaybeProp<BadgePrefix>,
    #[prop(optional, into)] postfix: MaybeProp<BadgePostfix>,
    #[prop(optional)] border: bool,
    #[prop(optional)] dismissable: bool,
    children: Children,
) -> impl IntoView {
    ...
    view! {
        <span id=id.clone() class=class_list!(BADGE_BASE_CLASSES, class, theme.base_class(), (theme.border_class(), border), ("gap-1", dismissable), prefix_classes(prefix), prefix_classes(postfix), size.class())>
            ...
            {if dismissable {
                Either::Left(view!{
                    <button type="button" class="inline-flex items-center p-0.5 text-sm bg-transparent rounded-xs hover:bg-neutral-tertiary" data-dismiss-target=target_id  aria_label="Remove">
...
impl BadgeSize {
    pub fn class(&self) -> &'static str {
        match self {
            BadgeSize::Normal => "text-xs px-2.5 py-0.5",
